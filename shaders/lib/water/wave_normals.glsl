...
float wave_height(vec2 position) {
...
vec3 wave_normal(vec2 pos, vec3 worldFaceNormal, float heightmapFactor) {
  // return worldFaceNormal;
  vec2 ex = vec2(WAVE_E, 0);
  float H = wave_height(pos.xy) * WAVE_DEPTH * heightmapFactor;
  ...
  vec3 waveNormal = normalize(
    cross(
      a -
        vec3(
          pos.x - WAVE_E,
          wave_height(pos.xy - ex.xy) * WAVE_DEPTH * heightmapFactor,
          pos.y
        ),
      a -
        vec3(
          pos.x,
          wave_height(pos.xy + ex.yx) * WAVE_DEPTH * heightmapFactor,
          pos.y + WAVE_E
        )
    )
  );
